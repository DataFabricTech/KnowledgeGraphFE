export class WindowLayer {
    get minScale() {
        return (Math.min(this._size.height / this._renderingLayer.size.height, this._size.width / this._renderingLayer.size.width) / 2);
    }
    get maxScale() {
        return 4;
    }
    constructor({ windowElement, eventHandler = {}, renderingLayer, scale, }) {
        this._windowCanvas = null;
    ...
    convertPointToRenderLayout(point) {
    ...
    movePosition(dPosition) {
        const { x, y } = dPosition;
        if (x)
            this.setPosition({ x: this._position.x + x * this._dpiRatio });
        if (y)
            this.setPosition({ y: this._position.y + y * this._dpiRatio });
    }
    ...
    scaleDownWithPoint(point, dRatio = 1) {
        const worldX = (point.x - this._position.x) / this._scale;
        const worldY = (point.y - this._position.y) / this._scale;
    ...
    fitToWindow() {
        this._scale = Math.min(this._size.height / this._renderingLayer.size.height, this._size.width / this._renderingLayer.size.width);
        this._position.x =
            (this._size.width - this._renderingLayer.size.width * this._scale) / 2;
        this._position.y =
            (this._size.height - this._renderingLayer.size.height * this._scale) / 2;
    ...
    renderMap() {
        ...
        const minMargin = 32;
        const graph = this._renderingLayer.offscreenCanvas;
        const scale = Math.min((map.width - minMargin * 2) / graph.width, (map.height - minMargin * 2) / graph.height);
    ...
    `;
        ...
        nav.querySelector(".graph_nav_plus_btn")?.addEventListener("click", () => {
            this.scaleUpWithPoint({ x: this._size.width / 2, y: this._size.height / 2 }, 30);
            this.render();
        });
        nav.querySelector(".graph_nav_minus_btn")?.addEventListener("click", () => {
            this.scaleDownWithPoint({ x: this._size.width / 2, y: this._size.height / 2 }, 30);
            this.render();
        });
        nav
